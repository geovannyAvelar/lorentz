"""
Lorentz API security regression tests.
...
# The TLS test talks to Lorentz's self-signed cert with verify=False. Resolve the
...
LORENTZ_URL_TLS = "https://127.0.0.1"
...
def _set_totp_secret(secret, sid):
    return requests.patch(
        ...
        headers={"X-Lorentz-SID": sid}, timeout=20)


def _login(pw, totp=None, base=LORENTZ_URL, verify=True, timeout=10):
    payload = {"password": pw}
    if totp is not None:
        payload["totp"] = totp
...
def _login_rate_limited(pw, totp=None, base=LORENTZ_URL, verify=True, attempts=6):
...
def setup_module(_mod):
    ...
    for _ in range(40):
        try:
            if requests.get(f"{LORENTZ_URL}/api/auth", timeout=5).status_code in (200, 401):
                break
...
class TestSessionCookieSecure:
    ...
    def test_http_login_cookie_has_no_secure_attribute(self):
        r = _login_rate_limited(PASSWORD, base=LORENTZ_URL)
    ...
    def test_https_login_cookie_has_secure_attribute(self):
        try:
            with warnings.catch_warnings():
                ...
                r = _login_rate_limited(PASSWORD, base=LORENTZ_URL_TLS, verify=False)
