...
if [[ -n "${test}" ]]; then
    if [[ -n "${cmake_args}" ]]; then
        cmake_args="${cmake_args} -DBUILD_TAR_REGRESSION=ON -DBUILD_GZIP_REGRESSION=ON -DBUILD_PTR_RESPONSE_REGRESSION=ON -DBUILD_DB_DRIVER_REGRESSION=ON -DBUILD_DB_LAYER_REGRESSION=ON"
    else
        cmake_args="-DBUILD_TAR_REGRESSION=ON -DBUILD_GZIP_REGRESSION=ON -DBUILD_PTR_RESPONSE_REGRESSION=ON -DBUILD_DB_DRIVER_REGRESSION=ON -DBUILD_DB_LAYER_REGRESSION=ON"
    fi
fi
