...
! THIS FILE IS AUTO-GENERATED FROM THE Lorentz REPO  !
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
-->
<!-- markdownlint-disable MD033 -->
# Lorentz Configuration Reference

This page documents the available options of `lorentz`. They are typically managed via the [TOML](https://toml.io/)-formatted configuration file `/etc/lorentz/lorentz.toml`. This file may be edited directly or you can use the command line (CLI) option, the web interface, the application programming interface (API) or environment variables.

Using the web interface, the API or the CLI is preferred as they can do error checking for you, trying to prevent any incompatible options which could prevent Lorentz from starting on a severely broken configuration.

To edit with the command line, use the format `--config key.name value`, e.g:

```text
sudo lorentz --config dns.dnssec true
```

!!! note "Environment Variables"
    **⚙️ Configuration Precedence**
    Every Lorentz setting in this file can be overridden using an environment variable.
    This is especially common in Docker deployments.
    Environment variable names follow the format:
    ```text
    LORENTZCONF_<section>_<key>
    ```
    For example:
    ```text
    LORENTZCONF_dns_upstreams
    LORENTZCONF_database_DBimport
    ```
...
"""
    ...
    for line in lines_iter:
        ...
        # Handle key-value pairs
            ...
            env_var = "LORENTZCONF_" + full_key.replace(".", "_")
            ...
            if example_display_value.strip().startswith("["):
                ...
                documentation.append(f"    sudo lorentz --config {full_key} '{array_str}'")
            else:
                documentation.append(f"    sudo lorentz --config {full_key} {example_display_value}")
            documentation.append("    ```")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print("Usage: python lorentz_toml_to_markdown.py <input.toml> <output.md>")
        sys.exit(1)
