	...
	for(unsigned int i = 0; i < CONFIG_ELEMENTS; i++)
	{
		...
		switch(conf_item->t)
		{
			...
			case CONF_STRING_ALLOCATED:
				...
				if(terminating && conf_item->f & FLAG_LORENTZ_LOG)
					continue;
				free(conf_item->v.s);
				conf_item->v.s = NULL;
...
void initConfig(struct config *conf)
{
	...
	conf->dns.upstreams.h = "Upstream DNS Servers to be used by Lorentz. If this is not set, Lorentz will not resolve any non-local queries.\n\n Example: [ \"8.8.8.8\", \"127.0.0.1#5335\", \"docker-resolver\" ]";
	...
	conf->dns.upstreams.f = FLAG_RESTART_LORENTZ;
	...
	conf->dns.ignoreLocalhost.h = "Should Lorentz hide queries made by localhost?";
	...
	conf->dns.showDNSSEC.h = "Should Lorentz analyze and show internally generated DNSSEC queries?";
	...
	conf->dns.analyzeOnlyAandAAAA.h = "Should Lorentz analyze *only* A and AAAA queries?";
	...
	conf->dns.lorentzPTR.k = "dns.lorentzPTR";
	conf->dns.lorentzPTR.h = "Controls whether and how Lorentz will reply with for address for which a local interface exists. Changing this setting causes Lorentz to restart.";
	{
		struct enum_options lorentzPTR[] =
		{
			{ get_ptr_type_str(PTR_NONE), "Lorentz will not respond automatically on PTR requests to local interface addresses. Ensure lorentz.lan and/or hostname records exist elsewhere." },
			...
			{ get_ptr_type_str(PTR_HOSTNAMEFQDN), "Serve the machine's hostname (see limitations above) as fully qualified domain by adding the local domain. If no local domain has been defined (config option dns.domain.name), Lorentz tries to query the domain name from the kernel using getdomainname(2). If this fails, Lorentz appends \".no_fqdn_available\" to the hostname." },
			{ get_ptr_type_str(PTR_LORENTZ), "Respond with \"lorentz.lan\"." }
		};
		CONFIG_ADD_ENUM_OPTIONS(conf->dns.lorentzPTR.a, lorentzPTR);
	}
	conf->dns.lorentzPTR.t = CONF_ENUM_PTR_TYPE;
	conf->dns.lorentzPTR.d.ptr_type = PTR_LORENTZ;
	conf->dns.lorentzPTR.f = FLAG_RESTART_LORENTZ;
	conf->dns.lorentzPTR.c = validate_stub; // Only type-based checking

	conf->dns.replyWhenBusy.k = "dns.replyWhenBusy";
	conf->dns.replyWhenBusy.h = "How should Lorentz handle queries when the gravity database is not available?";
	{
	...
	conf->dns.blockTTL.h = "Lorentz's internal TTL to be handed out for blocked queries in seconds. This settings allows users to select a value different from the dnsmasq config option local-ttl. This is useful in context of locally used hostnames that are known to stay constant over long times (printers, etc.).\n\n Note that large values may render whitelisting ineffective due to client-side caching of blocked queries.";
	...
	conf->dns.domainNeeded.f = FLAG_RESTART_LORENTZ;
	...
	conf->dns.expandHosts.f = FLAG_RESTART_LORENTZ;
	...
	conf->dns.bogusPriv.f = FLAG_RESTART_LORENTZ;
	...
	conf->dns.dnssec.f = FLAG_RESTART_LORENTZ;
	...
	conf->dns.interface.f = FLAG_RESTART_LORENTZ;
	...
	conf->dns.listeningMode.f = FLAG_RESTART_LORENTZ;
	...
	conf->dns.queryLogging.h = "Log DNS queries and replies to lorentz.log";
	conf->dns.queryLogging.t = CONF_BOOL;
	conf->dns.queryLogging.f = FLAG_RESTART_LORENTZ;
	...
	conf->dns.cnameRecords.f = FLAG_RESTART_LORENTZ;
	...
	conf->dns.port.f = FLAG_RESTART_LORENTZ;
	...
	conf->dns.localise.f = FLAG_RESTART_LORENTZ;
	conf->dns.localise.d.b = true;
	conf->dns.localise.c = validate_stub; // Only type-based checking

	conf->dns.revServers.k = "dns.revServers";
	conf->dns.revServers.h = "Reverse server (formerly called \"conditional forwarding\")\n\n If not configured as your DHCP server, Lorentz typically won't be able to determine the names of devices on your local network. As a result, tables such as Top Clients will only show IP addresses.\n\n One solution for this is to configure Lorentz to forward these requests to your DHCP server (most likely your router), but only for devices on your home network. To configure this we will need to know the IP address of your DHCP server and which addresses belong to your local network.\n\n Example: [ \"true,192.168.0.0/24,192.168.0.1,fritz.box\" ]";
	...
	conf->dns.domain.local.f = FLAG_RESTART_LORENTZ;
	...
	conf->dns.cache.size.f = FLAG_RESTART_LORENTZ;
	...
	conf->dns.cache.optimizer.f = FLAG_RESTART_LORENTZ;
	...
	conf->dns.cache.rrtype.h = "This is dnsmasq's --cache-rr option, which allows you to define which DNS record types should be cached by Lorentz. This option can take a comma-separated list of RR-types as input. The default value ANY caches all record types.";
	...
	conf->dns.cache.rrtype.f = FLAG_RESTART_LORENTZ;
	...
	conf->dns.blocking.active.h = "Should Lorentz block queries?";
	...
	conf->dns.blocking.mode.h = "How should Lorentz reply to blocked queries?";
	{
		struct enum_options blockingmode[] =
		{
			{ get_blocking_mode_str(MODE_NULL), "In NULL mode, which is both the default and recommended mode for Lorentz, blocked queries will be answered with the \"unspecified address\" (0.0.0.0 or ::). The \"unspecified address\" is a reserved IP address specified by RFC 3513 - Internet Protocol Version 6 (IPv6) Addressing Architecture, section 2.5.2." },
			{ get_blocking_mode_str(MODE_IP_NODATA_AAAA), "In IP-NODATA-AAAA mode, blocked queries will be answered with the local IPv4 addresses of your Lorentz. Blocked AAAA queries will be answered with NODATA-IPV6 and clients will only try to reach your Lorentz over its static IPv4 address." },
			{ get_blocking_mode_str(MODE_IP), "In IP mode, blocked queries will be answered with the local IP addresses of your Lorentz." },
	...
	conf->dns.blocking.edns.h = "Should Lorentz enrich blocked replies with EDNS0 information?";
	{
	...
	conf->dns.specialDomains.mozillaCanary.h = "Should Lorentz always reply with NXDOMAIN to A and AAAA queries of use-application-dns.net to disable Firefox automatic DNS-over-HTTP?\n\n This follows the recommendation on https://support.mozilla.org/en-US/kb/configuring-networks-disable-dns-over-https";
	...
	conf->dns.specialDomains.iCloudPrivateRelay.h = "Should Lorentz always reply with NXDOMAIN to A and AAAA queries of mask.icloud.com and mask-h2.icloud.com to disable Apple's iCloud Private Relay to prevent Apple devices from bypassing Lorentz?\n\n This follows the recommendation on https://developer.apple.com/support/prepare-your-network-for-icloud-private-relay";
	...
	conf->dns.specialDomains.designatedResolver.h = "Should Lorentz always reply with NODATA to all queries to zone resolver.arpa to prevent devices from bypassing Lorentz using Discovery of Designated Resolvers?\n\n This is based on recommendations at the end of RFC 9462, section 4.";
	...
	conf->dns.reply.host.force4.h = "Use a specific IPv4 address for the Lorentz host? By default, Lorentz determines the address of the interface a query arrived on and uses this address for replying to A queries with the most suitable address for the requesting client.\n\n This setting can be used to use a fixed, rather than the dynamically obtained, address when Lorentz responds to the following names:\n - \"lorentz.lan\"\n - \"<the device's hostname>\"\n - \"lorentz.lan.<local domain>\"\n - \"<the device's hostname>.<local domain>\"";
	...
	conf->dns.reply.host.v4.h = "Custom IPv4 address for the Lorentz host";
	...
	conf->dns.reply.host.force6.h = "Use a specific IPv6 address for the Lorentz host? See description for the IPv4 variant above for further details.";
	...
	conf->dns.reply.host.v6.h = "Custom IPv6 address for the Lorentz host";
	...
	conf->dns.reply.blocking.force4.h = "Use a specific IPv4 address in IP blocking mode? By default, Lorentz determines the address of the interface a query arrived on and uses this address for replying to A queries with the most suitable address for the requesting client.\n\n This setting can be used to use a fixed, rather than the dynamically obtained, address when Lorentz responds in the following cases:\n - IP blocking mode is used and this query is to be blocked\n - regular expressions with the ;reply=IP regex extension.";
	...
	conf->dns.rateLimit.count.h = "Rate-limited queries are answered with a REFUSED reply and not further processed by Lorentz.\n\n The default settings for Lorentz's rate-limiting are to permit no more than 1000 queries in 60 seconds. Both numbers can be customized independently. It is important to note that rate-limiting is happening on a per-client basis. Other clients can continue to use Lorentz while rate-limited clients are short-circuited at the same time.\n\n For this setting, both numbers, the maximum number of queries within a given time, and the length of the time interval (seconds) have to be specified. For instance, if you want to set a rate limit of 1 query per hour, the option should look like dns.rateLimit.count=1 and dns.rateLimit.interval=3600. The time interval is relative to when Lorentz has finished starting (start of the daemon + possible delay by DELAY_STARTUP) then it will advance in steps of the rate-limiting interval. If a client reaches the maximum number of queries it will be blocked until the end of the current interval. This will be logged to /var/log/lorentz/lorentz.log, e.g. Rate-limiting 10.0.1.39 for at least 44 seconds. If the client continues to send queries while being blocked already and this number of queries during the blocking exceeds the limit the client will continue to be blocked until the end of the next interval (lorentz.log will contain lines like Still rate-limiting 10.0.1.39 as it made additional 5007 queries). As soon as the client requests less than the set limit, it will be unblocked (Ending rate-limitation of 10.0.1.39).\n\n Rate-limiting may be disabled altogether by setting both values to zero (this results in the same behavior as before Lorentz v5.7).\n\n How many queries are permitted...";
	...
	conf->dhcp.active.f = FLAG_RESTART_LORENTZ;
	...
	conf->dhcp.start.f = FLAG_RESTART_LORENTZ;
	...
	conf->dhcp.end.f = FLAG_RESTART_LORENTZ;
	...
	conf->dhcp.router.f = FLAG_RESTART_LORENTZ;
	...
	conf->dhcp.netmask.h = "The netmask used by your Lorentz. For directly connected networks (i.e., networks on which the machine running Lorentz has an interface) the netmask is optional and may be set to an empty string (\"\"): it will then be determined from the interface configuration itself.\n\n For networks which receive DHCP service via a relay agent, we cannot determine the netmask itself, so it should explicitly be specified, otherwise Lorentz guesses based on the class (A, B or C) of the network address.\n\n Example: \"255.255.255.0\"";
	...
	conf->dhcp.netmask.f = FLAG_RESTART_LORENTZ;
	...
	conf->dhcp.leaseTime.f = FLAG_RESTART_LORENTZ;
	conf->dhcp.leaseTime.d.s = (char*)"";
	conf->dhcp.leaseTime.c = validate_str_no_newline;

	conf->dhcp.ipv6.k = "dhcp.ipv6";
	conf->dhcp.ipv6.h = "Should Lorentz make an attempt to also satisfy IPv6 address requests (be aware that IPv6 works a whole lot different than IPv4)";
	conf->dhcp.ipv6.t = CONF_BOOL;
	conf->dhcp.ipv6.f = FLAG_RESTART_LORENTZ;
	conf->dhcp.ipv6.d.b = false;
	conf->dhcp.ipv6.c = validate_stub; // Only type-based checking

	conf->dhcp.multiDNS.k = "dhcp.multiDNS";
	conf->dhcp.multiDNS.h = "Advertise DNS server multiple times to clients. Some devices will add their own proprietary DNS servers to the list of DNS servers, which can cause issues with Lorentz. This option will advertise the Lorentz DNS server multiple times to clients, which should prevent this from happening.";
	conf->dhcp.multiDNS.t = CONF_BOOL;
	conf->dhcp.multiDNS.f = FLAG_RESTART_LORENTZ;
	...
	conf->dhcp.rapidCommit.f = FLAG_RESTART_LORENTZ;
	...
	conf->dhcp.logging.f = FLAG_RESTART_LORENTZ;
	...
	conf->dhcp.hosts.f = FLAG_RESTART_LORENTZ;
	...
	conf->ntp.ipv4.active.h = "Should Lorentz act as network time protocol (NTP) server (IPv4)?";
	conf->ntp.ipv4.active.t = CONF_BOOL;
	conf->ntp.ipv4.active.f = FLAG_RESTART_LORENTZ;
	...
	conf->ntp.ipv4.address.f = FLAG_RESTART_LORENTZ;
	...
	conf->ntp.ipv6.active.h = "Should Lorentz act as network time protocol (NTP) server (IPv6)?";
	conf->ntp.ipv6.active.t = CONF_BOOL;
	conf->ntp.ipv6.active.f = FLAG_RESTART_LORENTZ;
	...
	conf->ntp.ipv6.address.f = FLAG_RESTART_LORENTZ;
	...
	conf->ntp.sync.active.h = "Should Lorentz try to synchronize the system time with an upstream NTP server?";
	conf->ntp.sync.active.t = CONF_BOOL;
	conf->ntp.sync.active.f = FLAG_RESTART_LORENTZ;
	...
	conf->ntp.sync.rtc.set.h = "Should Lorentz update a real-time clock (RTC) if available?";
	...
	conf->resolver.resolveIPv6.h = "Should Lorentz try to resolve IPv6 addresses to hostnames?";
	...
	conf->resolver.resolveIPv4.h = "Should Lorentz try to resolve IPv4 addresses to hostnames?";
	...
	conf->resolver.refreshNames.h = "With this option, you can change how (and if) hourly PTR requests are made to check for changes in client and upstream server hostnames.";
	{
		...
		{
			{ get_refresh_hostnames_str(REFRESH_IPV4_ONLY), "Do hourly PTR lookups only for IPv4 addresses. This is the new default since Lorentz v5.3.2. It should resolve issues with more and more very short-lived PE IPv6 addresses coming up in a lot of networks." },
			{ get_refresh_hostnames_str(REFRESH_ALL), "Do hourly PTR lookups for all addresses. This was the default until Lorentz v5.3(.1). It has been replaced as it can create a lot of PTR queries for those with many IPv6 addresses in their networks." },
	...
	conf->database.DBinterval.h = "How often do we store queries in Lorentz's database [seconds]?";
	...
	conf->database.useWAL.h = "Should Lorentz enable Write-Ahead Log (WAL) mode for the on-disk query database (configured via files.database)?\n\n It is recommended to leave this setting enabled for performance reasons. About the only reason to disable WAL mode is if you are experiencing specific issues with it, e.g., when using a database that is accessed from multiple hosts via a network share. When this setting is disabled, Lorentz will use SQLite3's default journal mode (rollback journal in DELETE mode).";
	conf->database.useWAL.t = CONF_BOOL;
	// Note: We would not necessarily need to restart Lorentz when this setting
	...
	conf->database.useWAL.f = FLAG_RESTART_LORENTZ;
	...
	conf->database.forceDisk.h = "Should Lorentz force the use of disk storage for the history database? By default, Lorentz uses an in-memory database for much improved performance when browsing the history from the dashboard. However, on systems with very limited RAM and only occasional usage of the web interface, it may be useful to force the use of disk storage instead of holding everything in memory.\n\n Note that using disk storage *will* reduce performance, especially on systems with slow storage media (e.g., SD cards).";
	conf->database.forceDisk.t = CONF_BOOL;
	conf->database.forceDisk.f = FLAG_RESTART_LORENTZ;
	...
	conf->database.network.parseARPcache.h = "Should Lorentz analyze the local ARP cache? When disabled, client identification and the network table will stop working reliably.";
	...
	conf->webserver.domain.f = FLAG_RESTART_LORENTZ;
	conf->webserver.domain.d.s = (char*)"lorentz.lan";
	...
	conf->webserver.acl.f = FLAG_RESTART_LORENTZ;
	...
	conf->webserver.port.f = FLAG_RESTART_LORENTZ;
	...
	conf->webserver.threads.f = FLAG_RESTART_LORENTZ;
	...
	conf->webserver.headers.f = FLAG_RESTART_LORENTZ;
	...
	conf->webserver.advancedOpts.f = FLAG_RESTART_LORENTZ | FLAG_API_READ_ONLY;
	...
	conf->webserver.tls.validity.h = "Number of days the automatically generated self-signed TLS/SSL certificate will be valid for.\n\n Defaults to 47 days. A minimum of 7 days is enforced.\n Some devices may enforce shorter validity ranges. Note that defining a lower validity range may require you to accept the self-signed certificate more often in your browser.\n Lorentz will regenerate certificates it created itself two days prior to expiration. If you are using your own certificate, you need to regenerate it yourself. In this case, it is advised to set the validity range to 0 days, so that Lorentz does not try to regenerate your certificate. If you set the validity range to 0 days and still try to generate a certificate, Lorentz will set a fixed validity range of roughly 30 years for the certificate.";
	...
	conf->webserver.session.restore.h = "Should Lorentz backup and restore sessions from the database?\n\n This is useful if you want to keep your sessions after a restart of the web interface.";
	...
	conf->webserver.paths.webroot.f = FLAG_RESTART_LORENTZ | FLAG_API_READ_ONLY;
	...
	conf->webserver.paths.webhome.h = "Sub-directory of the root containing the web interface\n\n This decides where the web server serves the interface from, so it cannot be set through the API. Set it in "GLOBALTOMLPATH", through an environment variable, or with \"lorentz --config\" - all of which require access to the host.";
	...
	conf->webserver.paths.webhome.f = FLAG_RESTART_LORENTZ | FLAG_API_READ_ONLY;
	...
	conf->webserver.paths.prefix.h = "Prefix where the web interface is served\n\n This is useful when you are using a reverse proxy serving the web interface, e.g., at http://<ip>/lorentz/admin/ instead of http://<ip>/admin/. In this example, the prefix would be \"/lorentz\". Note that the prefix has to be stripped away by the reverse proxy, e.g., for traefik:\n - traefik.http.routers.lorentz.rule=PathPrefix(`/lorentz`)\n - traefik.http.middlewares.lorentzhttp.stripprefix.prefixes=/lorentz\n The prefix should start with a slash. If you don't use a prefix, leave this field empty. Setting this field to an incorrect value may result in the web interface not being accessible.\n Don't use this setting if you are not using a reverse proxy!\n\n This decides where the web server serves the interface from, so it cannot be set through the API. Set it in "GLOBALTOMLPATH", through an environment variable, or with \"lorentz --config\" - all of which require access to the host.";
	...
	conf->webserver.paths.prefix.f = FLAG_RESTART_LORENTZ | FLAG_API_READ_ONLY;
	...
	conf->webserver.interface.theme.h = "Theme used by the Lorentz web interface";
	{
	...
	conf->webserver.api.max_sessions.f = FLAG_RESTART_LORENTZ;
	...
	conf->webserver.api.prettyJSON.h = "Should Lorentz prettify the API output (add extra spaces, newlines and indentation)?";
	...
	conf->webserver.api.pwhash.a = cJSON_CreateStringReference("A valid Lorentz password hash");
	...
	conf->webserver.api.password.h = "Lorentz web interface and API password. When set to something different than \""PASSWORD_VALUE"\", this property will compute the corresponding password hash to set webserver.api.pwhash";
	conf->webserver.api.password.a = cJSON_CreateStringReference("A valid Lorentz password");
	...
	conf->webserver.api.cli_pw.h = "Should Lorentz create a temporary CLI password?\n\n This password is stored in clear in /etc/lorentz and can be used by the CLI (lorentz ...  commands) to authenticate against the API. Note that the password is only valid for the current session and regenerated on each Lorentz restart. Sessions initiated with this password cannot modify the Lorentz configuration (change passwords, etc.) for security reasons but can still use the API to query data and manage lists.";
	conf->webserver.api.cli_pw.t = CONF_BOOL;
	conf->webserver.api.cli_pw.f = FLAG_RESTART_LORENTZ;
	...
	conf->webserver.api.maxHistory.f = FLAG_RESTART_LORENTZ; // Restart Lorentz to import more data in case of enlarging of this value
	...
	conf->webserver.api.temp.limit.h = "Which upper temperature limit should be used by Lorentz? Temperatures above this limit will be shown as \"hot\". The number specified here is in the unit defined below";
	...
	conf->webserver.api.temp.unit.h = "Which temperature unit should be used for temperatures processed by Lorentz?";
	{
	...
	conf->files.tmp_db.a = cJSON_CreateStringReference("Any Lorentz database");
	conf->files.tmp_db.t = CONF_STRING;
	conf->files.tmp_db.f = FLAG_RESTART_LORENTZ;
	conf->files.tmp_db.d.s = (char*)"/etc/lorentz/lorentz-tmp.db";
	...
	conf->files.gravity_tmp.f = FLAG_RESTART_LORENTZ;
	...
	conf->files.macvendor.a = cJSON_CreateStringReference("Any Lorentz macvendor database");
	conf->files.macvendor.t = CONF_STRING;
	conf->files.macvendor.d.s = (char*)"/etc/lorentz/macvendor.db";
	conf->files.macvendor.c = validate_filepath;

	conf->files.pcap.k = "files.pcap";
	conf->files.pcap.h = "An optional file containing a pcap capture of the network traffic. This file is used for debugging purposes only. If you don't know what this is, you don't need it.\n\n Setting this to an empty string disables pcap recording. The file must be writable by the user running Lorentz (typically lorentz). Failure to write to this file will prevent the DNS resolver from starting. The file is appended to if it already exists.";
	conf->files.pcap.a = cJSON_CreateStringReference("Any writable pcap file");
	conf->files.pcap.t = CONF_STRING;
	conf->files.pcap.f = FLAG_RESTART_LORENTZ;
	conf->files.pcap.d.s = (char*)"";
	conf->files.pcap.c = validate_filepath_empty;

	// sub-struct files.log
	// conf->files.log.lorentz is set in a separate function (getLogFilePath)
	...
	conf->files.log.dnsmasq.f = FLAG_RESTART_LORENTZ;
	conf->files.log.dnsmasq.d.s = (char*)"/var/log/lorentz/lorentz.log";
	...
	conf->files.log.webserver.f = FLAG_RESTART_LORENTZ;
	conf->files.log.webserver.d.s = (char*)"/var/log/lorentz/webserver.log";
	conf->files.log.webserver.c = validate_filepath;

	// struct misc
	conf->misc.privacylevel.k = "misc.privacylevel";
	conf->misc.privacylevel.h = "Using privacy levels you can specify which level of detail you want to see in your Lorentz statistics. Changing this setting will trigger a restart of Lorentz";
	{
	...
	conf->misc.delay_startup.h = "During startup, in some configurations, network interfaces appear only late during system startup and are not ready when Lorentz tries to bind to them. Therefore, you may want Lorentz to wait a given amount of time before trying to start the DNS resolver.\n\n This setting takes any integer value between 0 and 300 seconds. To prevent delayed startup while the system is already running and Lorentz is restarted, the delay only takes place within the first 180 seconds (hard-coded) after booting.";
	...
	conf->misc.nice.h = "Set niceness of lorentz. Defaults to -10 and can be disabled altogether by setting a value of -999. The nice value is an attribute that can be used to influence the CPU scheduler to favor or disfavor a process in scheduling decisions.\n\n The range of the nice value varies across UNIX systems. On modern Linux, the range is -20 (high priority = not very nice to other processes) to +19 (low priority).";
	...
	conf->misc.etc_dnsmasq_d.h = "Should Lorentz load additional dnsmasq configuration files from /etc/dnsmasq.d/?\n\n Warning: This is an advanced setting and should only be used with care.\n Incorrectly formatted or config files specifying options which can only be defined once can result in conflicts with the automatic configuration of Lorentz (see "DNSMASQ_PH_CONFIG") and may stop DNS resolution from working.";
	conf->misc.etc_dnsmasq_d.t = CONF_BOOL;
	conf->misc.etc_dnsmasq_d.f = FLAG_RESTART_LORENTZ;
	...
	conf->misc.dnsmasq_lines.f = FLAG_RESTART_LORENTZ | FLAG_API_READ_ONLY;
	...
	conf->misc.hide_dnsmasq_warn.h = "Should Lorentz hide warnings coming from dnsmasq?\n\n By default, Lorentz reports warnings coming from the embedded dnsmasq DNS server to the Lorentz log file. These warnings can be useful to identify misconfigurations or problems with the DNS server. However, some warnings may be harmless and can be ignored in certain setups. Enabling this setting will hide all dnsmasq warnings.";
	...
	conf->misc.hide_connection_error.h = "Should Lorentz hide network connection errors?\n\n By default, Lorentz reports network connection errors (e.g., Connection prematurely closed by remote server) to the Lorentz log file. These warnings can be useful to identify intermittent network problems or general problem with upstream servers. However, in some setups, these warnings may be expected (e.g. due to low-quality Internet connectivity) and cannot be fixed. Enabling this setting will hide all connection warnings.";
	...
	conf->misc.check.load.h = "Lorentz is very lightweight on resources. Nevertheless, this does not mean that you should run Lorentz on a server that is otherwise extremely busy as queuing on the system can lead to unnecessary delays in DNS operation as the system becomes less and less usable as the system load increases because all resources are permanently in use. To account for this, Lorentz regularly checks the system load. To bring this to your attention, Lorentz warns about excessive load when the 15 minute system load average exceeds the number of cores.\n\n This check can be disabled with this setting.";
	...
	conf->misc.check.disk.h = "Lorentz stores its long-term history in a database file on disk. Furthermore, Lorentz stores log files. By default, Lorentz warns if usage of the disk holding any crucial file exceeds 90%. You can set any integer limit between 0 to 100 (interpreted as percentages) where 0 means that checking of disk usage is disabled.";
	...
	conf->misc.check.shmem.h = "Lorentz stores history in shared memory to allow inter-process communication with forked dedicated TCP workers. If Lorentz runs out of memory, it cannot continue to work as queries cannot be analyzed any further. Hence, Lorentz checks if enough shared memory is available on your system and warns you if this is not the case.\n By default, Lorentz warns if the shared-memory usage exceeds 90%. You can set any integer limit between 0 to 100 (interpreted as percentages) where 0 means that checking of shared-memory usage is disabled.";
	...
	conf->debug.networking.h = "Prints a list of the detected interfaces on the startup of lorentz. Also, prints whether these interfaces are IPv4 or IPv6 interfaces.";
	...
	conf->debug.queries.h = "Print extensive query information (domains, types, replies, etc.). This has always been part of the legacy debug mode of lorentz.";
	...
	conf->debug.regex.h = "Controls if LORENTZDNS should print extended details about regex matching into lorentz.log.";
	...
	conf->debug.caps.h = "Print information about capabilities granted to the lorentz process. The current capabilities are printed on receipt of SIGHUP, i.e., the current set of capabilities can be queried without restarting lorentz (by setting DEBUG_CAPS=true and thereafter sending killall -HUP lorentz).";
	...
	conf->debug.vectors.h = "Lorentz uses dynamically allocated vectors for various tasks. This config option enables extensive debugging information such as information about allocation, referencing, deletion, and appending.";
	...
	conf->debug.events.h = "Log information regarding Lorentz's embedded event handling queue.";
	...
	conf->debug.inotify.h = "Debug monitoring of /etc/lorentz filesystem events";
	...
	conf->debug.timing.h = "Print timing information from various parts of Lorentz";
	...
	conf->debug.performance.h = "Log gravity lookup and Lorentz DNS cache performance statistics every 5 minutes. For each operation type (gravity, antigravity, denylist, allowlist), reports the call count, average and maximum latency, and percentage of slow queries (>1 ms). Also reports the Lorentz cache hit/miss ratio, indicating how often gravity.db is queried at all.";
	...
	for(unsigned int i = 0; i < CONFIG_ELEMENTS; i++)
	{
		...
		if(conf_item != &conf->files.log.lorentz)
		...
		const size_t envkey_size = strlen(conf_item->k) + sizeof(LORENTZCONF_PREFIX);
		conf_item->e = calloc(envkey_size, sizeof(char));

		// Build env key to look for
		strcpy(conf_item->e, LORENTZCONF_PREFIX);
...
bool migrate_config_v6(void)
{
	...
	log_info("Migrating config to Lorentz v6.0 format");
	...
	const char *path = "";
	if((path = readLorentzlegacy(&config)) != NULL)
	{
		const char *target = MIGRATION_TARGET_V6"/lorentz.conf";
	...
	// setupVars.conf
	importsetupVarsConf();
	// 04-lorentz-static-dhcp.conf
	read_legacy_dhcp_static_config();
	// 05-lorentz-custom-cname.conf
...
bool readLorentzconf(struct config *conf, const bool rewrite)
{
	...
	// If we cannot parse /etc/lorentz.toml (due to missing or invalid syntax),
	// we try to read the rotated files in /etc/lorentz/config_backup starting at
	// the most recent one and going back in time until we find a valid config
	for(unsigned int i = 0; i < MAX_ROTATIONS; i++)
	{
		toml_datum_t toml = { 0 };
		if(readLorentztoml(NULL, conf, toml, rewrite, NULL, i, false, NULL))
		{
	...
	// When we reach this point but the Lorentz TOML config file exists, it may
...
bool getLogFilePath(bool try_read)
{
	...
	config.files.log.lorentz.k = "files.log.lorentz";
	config.files.log.lorentz.h = "The location of Lorentz's log file";
	config.files.log.lorentz.a = cJSON_CreateStringReference("any writable file");
	config.files.log.lorentz.t = CONF_STRING;
	config.files.log.lorentz.d.s = (char*)"/var/log/lorentz/lorentz.log";
	config.files.log.lorentz.v.s = config.files.log.lorentz.d.s;
	config.files.log.lorentz.c = validate_filepath;
	config.files.log.lorentz.f = FLAG_LORENTZ_LOG;
...
void reread_config(void)
{
	...
	log_info("Reloading config due to lorentz.toml change");
	...
	bool restart = false;
	toml_datum_t toml = { 0 };
	if(readLorentztoml(&config, &conf_copy, toml, true, &restart, 0, false, NULL))
	{
		...
		// Check if privacy level was reduced. If so, we need to restart Lorentz
		if(conf_copy.misc.privacylevel.v.privacy_level < config.misc.privacylevel.v.privacy_level)
		{
			log_info("Privacy level was reduced, restarting Lorentz");
			// We need to restart Lorentz
			restart = true;
		}

		// Replace config struct used by Lorentz by newly loaded
	...
	// If we need to restart Lorentz, we do so now
	if(restart)
		restart_lorentz("lorentz.toml change");
}
 ...
 * directory to the user running the Lorentz program.
 *
...
bool create_migration_target_v6(void)
{
	...
	{
		// Change ownership of the directory to the user running Lorentz
